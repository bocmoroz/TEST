...
@SpringBootTest(classes = {ProductController.class, EntityRequestValidationService.class})
...
public class ProductControllerTest {
    ...
    public void getProductsTestSuccess() {
        ...
        ResponseEntity<ResponseDto<List<Product>>> response = productController.getProducts();
    ...
    public void getProductByArticleTestSuccess() {
        ...
        ResponseEntity<ResponseDto<Product>> response = productController.getProductByArticle(product.getArticul());

        Assert.assertEquals(HttpStatus.OK, response.getStatusCode());
        Assert.assertNotNull(response.getBody());
        Assert.assertEquals(0, response.getBody().getStatus());
        Assert.assertEquals("Продукт успешно получен!", response.getBody().getMessage());
        Assert.assertEquals(response.getBody().getBody(), product);
    }
    ...
    public void getProductByArticleTestFailInternalError() {
        Product product = new Product();
        product.setArticul("артикул");
        Mockito.when(productService.getProductByArticle(product.getArticul()))
    ...
    public void addNewProductTestSuccess(){
        ...
        ResponseEntity<ResponseDto<Product>> response = productController.addNewProduct(product);
    ...
    public void addNewProductTestFailAlreadyExist(){
        ...
        product.setArticul("артикул");
        product.setName("имя");
        Mockito.when(productService.addNewProduct(product)).thenThrow(new ProductValidationException("Такой артикул уже существует!"));
        ...
        Assert.assertNotNull(response.getBody());
        Assert.assertEquals(1, response.getBody().getStatus());
        Assert.assertEquals("Такой артикул уже существует!", response.getBody().getMessage());
        Assert.assertNull(response.getBody().getBody());
    ...
    public void addNewProductTestFailInternalError(){
        ...
        product.setArticul("артикул");
        product.setName("имя");
        Mockito.when(productService.addNewProduct(product)).thenThrow(new RuntimeException("Внутренняя ошибка"));
